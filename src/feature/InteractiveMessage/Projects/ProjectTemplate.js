...
import { ProjectNadeulImg, ProjectSsafyImg, BoxContent, ProjectSsgameImg, ProjectOndaImg } from "./styles";
...
export default function ProjectTemplate({project}) {
    ...
    const projectImg = () => {
        ...
        else if(project.title === 'SSGAME') return <ProjectSsgameImg />
        else if(project.title === '온:다') return <ProjectOndaImg />
        ...
        <React.Fragment>
            ...
            >
                <Box sx={{ ...style, width: "50vw" }}>
                    <BoxContent>
                        <h2 id="child-modal-title">{project.title}</h2>
                            {projectImg()}
                        <br />
